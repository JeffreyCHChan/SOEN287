...
let itemList3 = (JSON.parse(localStorage.getItem('itemList3')) || []);
...
aTCButtonDOM.forEach(itemDescription => {
    itemDescription.addEventListener('click' , () => {
        ...
        const inCheck = (itemList3.filter(itemInfo => (itemInfo.name === item.name)).length > 0);
        if (inCheck === false){
            insertItem(item);

        itemList3.push(item);
        //To allow refreshing the page and keeping the content.
        localStorage.setItem('itemList3' , JSON.stringify(itemList3) );
        totalPrice();
        buttonHandler(item);
        ...
        individualInfo.querySelector('[data-action="DecreaseItem"]').addEventListener('click', () => {
            itemList3.forEach(itemInfo => {
                if(itemInfo.name === item.name) {
                    ...
                    itemList3 = itemList3.filter(itemInfo => itemInfo.name !== item.name);
                    localStorage.setItem('itemList3' , JSON.stringify(itemList3) );
                    totalPrice();
                    }
                }
...
function cartadd(){
var c=localStorage.getItem('itemList3')
